...
beverages_data <- list(
  "Cerveza (Caña/Tercio, 300ml, 5%)" = list(vol_ml = 300, abv = 5, etiqueta_ube = "~1 UBE"),
  "Vino (Copa, 125ml, 13%)" = list(vol_ml = 125, abv = 13, etiqueta_ube = "~1.3 UBE"),
  "Destilado (Combinado/Chupito, 50ml, 40%)" = list(vol_ml = 50, abv = 40, etiqueta_ube = "~1.6 UBE"),
  "Vermut/Jerez (Copa, 70ml, 15%)" = list(vol_ml = 70, abv = 15, etiqueta_ube = "~0.8 UBE")
)

# Calcular gramos de etanol para cada bebida
for (bev_name in names(beverages_data)) {
  bev <- beverages_data[[bev_name]]
  beverages_data[[bev_name]]$gramos_etanol <- round(bev$vol_ml * (bev$abv / 100) * DENSIDAD_ETANOL, 1)
}
...
ui <- page_sidebar(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .nav-tabs .nav-link.active, .nav-tabs .nav-item.show .nav-link {
        background-color: #3498DB !important; color: white !important; border-color: #3498DB #3498DB #fff;
      }
      ...
      /* Estilo para los h5 dentro de la card de cálculos detallados */
      #calculos_detallados_ui h5 { margin-top: 1rem; margin-bottom: 0.5rem; font-weight: bold; color: #2C3E50; }
      #calculos_detallados_ui hr { margin-top: 0.5rem; margin-bottom: 1rem; }
      #calculos_detallados_ui p { margin-bottom: 0.3rem; }
    "))
  ),
  ...
  page_fluid(
    ...
    layout_column_wrap(width = "100%", heights_equal = "row",
                       ...
                       card(class = "shadow-sm", card_header(h4("Important Notes and Interpretation")),
                            card_body(
                              tags$ul(
                                tags$li("Retrograde extrapolation is a mathematical process."),
                                tags$li("It´s based on the Widmark formula: ", tags$code("BAC_event = BAC_test + (elimination rate * time between event and test)")),
                                tags$li("The results show a ", tags$strong("possible range"),", due to individual variability in the rate of alcohol elimination (β)."),
                                tags$li("Factors such as gender, weight, food intake, drinking pattern, and health status can influence the actual rate of elimination and are not considered in this simplified calculation."),
                                tags$li("This tool is intended to support expert analysis, but it has no legal validity on its own.")
                              ),
                              h5("Reference:"),
...
server <- function(input, output, session) {
  ...
  iv$add_rule("bac_medido", sv_required("Debe ingresar un valor para BAC medido."))
  iv$add_rule("bac_medido", sv_gte(0, "El BAC medido no puede ser negativo."))
  iv$add_rule("hora_medicion", sv_required("La hora de medición es obligatoria."))
  iv$add_rule("hora_medicion", sv_regex("^([01]?[0-9]|2[0-3]):[0-5][0-9]$", "Hora de medición: formato HH:MM."))
  iv$add_rule("hora_evento", sv_required("La hora del evento es obligatoria."))
  iv$add_rule("hora_evento", sv_regex("^([01]?[0-9]|2[0-3]):[0-5][0-9]$", "Hora del evento: formato HH:MM."))
  ...
  iv$add_rule("fecha_evento", function(value) {
    ...
    tiempo_medicion <- ymd_hm(paste(input$fecha_medicion, input$hora_medicion), tz = Sys.timezone())
    tiempo_evento <- ymd_hm(paste(input$fecha_evento, input$hora_evento), tz = Sys.timezone())
    if (is.na(tiempo_medicion) || is.na(tiempo_evento)) {
      return("Error al combinar fechas y horas. Verifique que las fechas sean válidas.")
  ...
  resultado <- eventReactive(input$calcular, {
    ...
    tiempo_medicion_val <- ymd_hm(paste(input$fecha_medicion, input$hora_medicion), tz = Sys.timezone())
  ...
  output$resultados_ui <- renderUI({
    ...
    req(res)          
    tagList(
      p(strong("Time between event and time of blood draw: "), paste(res$horas, "hours")),
  ...
  grafico <- reactive({
    ...
    ggplot(points_df, aes(x = time, y = bac)) +
      ...
      geom_text(aes(label = label_text), vjust = -1.2, size = 3.8, check_overlap = FALSE, fontface = "bold") + 
      scale_x_datetime(name = "Time", breaks = sort(unique(points_df$time)),
                       labels = function(brks) {
                         sapply(brks, function(t) {
                           base_format <- format(t, "%H:%M\n%d/%m/%Y")
                           if (t == res$tiempo_evento_val) { paste0(base_format, "\n(T Event)") }
      ...
      scale_color_manual(values = c("Analítico" = "#0072B2", "Extrapolado (Mín)" = "#E69F00", "Extrapolate Max" = "#D55E00")) + 
      labs(color = "Resultado:", shape = "Resultado:") +
      theme_bw(base_size = 14) + 
      theme(legend.position = "bottom", axis.title = element_text(face = "bold", size = 12),
            axis.text.x = element_text(angle = 0, hjust = 0.5, size = 10), axis.text.y = element_text(size = 10),
  ...
  output$calculos_detallados_ui <- renderUI({
    ...
    tagList(
      ...
      tags$h5("Calculation of the lower limit of the estimated range"),
      tags$p(paste0("Using the Minimum Elimination Rate (beta_min) = ", res$beta_min_val, " g/L/hour:")),
      tags$p(tags$code(
        paste0("BAC_event_min = ", 
      ...
      tags$p(tags$strong(
        paste0("BAC_event_min = ", res$bac_min |> formatC(digits = 2, format = "f"), " g/L")
      )),
      tags$hr(),
      
      tags$h5("Calculation of the higher limit of the estimated range"),
      tags$p(paste0("Using the Maximum Elimination Rate (beta_max) = ", res$beta_max_val, " g/L/hour:")),
      tags$p(tags$code(
        paste0("BAC_event_max = ", res$bac_medido_val |> formatC(digits = 2, format = "f"), " g/L + (", 
               res$beta_max_val |> formatC(digits = 2, format = "f"), " g/L/hour * ", res$horas, " hours)")
      )),
      tags$p(HTML(paste0(
        "&nbsp;&nbsp;&nbsp;&nbsp; = ", res$bac_medido_val |> formatC(digits = 2, format = "f"), " g/L + ", 
      ...
      tags$p(tags$strong(
        paste0("BAC_event_max = ", res$bac_max |> formatC(digits = 2, format = "f"), " g/L")
      ))
    )
  })
  ...
  output$sintomas_output <- renderUI({
    ...
    sintomas_info <- list( 
      list(rango = c(-Inf, 0.29), titulo = "BAC < 0.3 g/L (Muy Bajo - Leve)", efectos = c("La mayoría de las personas no muestran efectos obvios o estos son muy leves.", "Ligera intensificación del humor."), class = "alert-light"),
      list(rango = c(0.3, 0.59), titulo = "BAC 0.3 - 0.59 g/L (Bajo - Euforia Leve)", efectos = c("Sensación de bienestar, relajación, euforia leve.", "Reducción de la inhibición.", "Disminución de la atención y control."), class = "alert-info"),
      list(rango = c(0.6, 0.99), titulo = "BAC 0.6 - 0.99 g/L (Moderado - Excitación)", efectos = c("Deterioro del juicio.", "Tiempo de reacción aumentado.", "Pérdida de coordinación muscular."), class = "alert-primary"),
      list(rango = c(1.0, 1.49), titulo = "BAC 1.0 - 1.49 g/L (Alto - Confusión)", efectos = c("Confusión mental, desorientación.", "Alteraciones sensoriales.", "Ataxia severa."), class = "alert-warning"),
      list(rango = c(1.5, 2.49), titulo = "BAC 1.5 - 2.49 g/L (Muy Alto - Estupor)", efectos = c("Estupor.", "Incapacidad para mantenerse de pie.", "Vómitos, riesgo de aspiración."), class = "alert-danger"),
      list(rango = c(2.5, 3.49), titulo = "BAC 2.5 - 3.49 g/L (Severo - Coma)", efectos = c("Coma.", "Reflejos deprimidos.", "Depresión respiratoria."), class = "alert-danger fw-bold"),
      list(rango = c(3.5, Inf), titulo = "BAC ≥ 3.5 g/L (Potencialmente Letal)", efectos = c("Depresión respiratoria severa/paro.", "Paro cardíaco.", "Muerte."), class = "alert-danger fw-bolder")
    )
    info_seleccionada <- NULL
    for (item in sintomas_info) { if (bac >= item$rango[1] && bac <= item$rango[2]) { info_seleccionada <- item; break } }
    if (!is.null(info_seleccionada)) {
      tags$div(class = paste("card shadow-sm mt-3"), 
               tags$div(class = paste("card-header fs-5", info_seleccionada$class), info_seleccionada$titulo), 
               tags$div(class = "card-body",
                        tags$ul(class = "list-group list-group-flush", lapply(info_seleccionada$efectos, function(efecto) { tags$li(class = "list-group-item", efecto) })),
                        tags$p(class="mt-3 small text-muted", "Fuente: Información general adaptada de fuentes toxicológicas y de salud pública. La respuesta individual al alcohol puede variar.")
               ))
    } else { tags$p(class="alert alert-secondary mt-3", "No se encontró información para el nivel de BAC seleccionado o el nivel es 0.") }
  })
  ...
  reporte <- reactive({
    ...
    # Título
    doc <- body_add_par(doc, "Report Retrograde extrapolation alcohol", style = "heading 1")
    
    # Datos de entrada
    doc <- body_add_par(doc, paste("Result of the blood test:", resultado()$bac_medido_val, "g/L"), style = "Normal")
    doc <- body_add_par(doc, paste("Elapsed Time:", resultado()$horas, "hours"), style = "Normal")
    ...
    # browser()
    doc <- body_add_par(doc, "Estimated BAC at time of incident (g/L):", style = "heading 2")
    
    doc <- body_add_par(doc, 
                        paste0("Minimum Elimination Rate (beta_min)", BETA_MIN, "): ", 
                               resultado()$bac_min |> formatC(digits = 2, format = "f"), " g/L"), 
                        style = "Normal")
    doc <- body_add_par(doc, 
                        paste0("Maximum Elimination Rate (beta_max)", BETA_MAX, "): ", 
    ...
    doc <- body_add_par(doc, "Retrograde extrapolation plot:", style = "heading 2")
