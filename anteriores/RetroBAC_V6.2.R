      ...
      card(
        ...
        card_body(
          tags$ul(
            ...
            tags$li(
              "It´s based on the Widmark formula: ",
              tags$code(
                "BAC_event = BAC_test + (elimination rate * time between event and test)"
              )
            ),
            tags$li(
              "The results show a ",
              tags$strong("possible range"),
              ", due to individual variability in the rate of alcohol elimination (β)."
            ),
            tags$li(
              "Factors such as gender, weight, food intake, drinking pattern, and health status can influence the actual rate of elimination and are not considered in this simplified calculation."
            ),
            tags$li(
              "This tool is intended to support expert analysis, but it has no legal validity on its own."
            )
          ),
          h5("Reference:"),
          tags$ul(tags$a(
...
server <- function(input, output, session) {
  ...
  output$resultados_ui <- renderUI({
    ...
    tagList(
      p(
        strong("Time between event and time of blood draw: "),
  ...
  output$calculos_detallados_ui <- renderUI({
    ...
    tagList(
      ...
      tags$p(paste0(
        "Using the Minimum Elimination Rate (beta_min) = ",
        res$beta_min_val,
        " g/L/hour:"
      )),
      tags$p(tags$code(
      ...
      tags$p(paste0(
        "Using the Maximum Elimination Rate (beta_max) = ",
        res$beta_max_val,
        " g/L/hour:"
      )),
      tags$p(tags$code(
  ...
  reporte <- reactive({
    ...
    doc <- body_add_par(
      doc,
      "Report Retrograde extrapolation alcohol",
      style = "heading 1"
    )
    ...
    doc <- body_add_par(
      doc,
      paste("Result of the blood test:", resultado()$bac_medido_val, "g/L"),
      style = "Normal"
    )
    doc <- body_add_par(
      doc,
      paste("Elapsed Time:", resultado()$horas, "hours"),
      style = "Normal"
    )
    ...
    doc <- body_add_par(
      doc,
      "Estimated BAC at time of incident (g/L):",
      style = "heading 2"
    )
    ...
    doc <- body_add_par(
      doc,
      paste0(
        "Minimum Elimination Rate (beta_min)",
        BETA_MIN,
        "): ",
    ...
    doc <- body_add_par(
      doc,
      paste0(
        "Maximum Elimination Rate (beta_max)",
        BETA_MAX,
        "): ",
